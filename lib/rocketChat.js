...
function RocketChatClient(protocol, host, port, username, password, onConnected, verbose, options) {
    let _authToken;
    let _userId;
    let basepath = "";
    ...
    const restClient = new net.RestClient(protocol, host, port, basepath + "/api/v1/", verbose, options);
    ...
    this.login = async function login(username, password, realtime){
        return await self.authentication.login(username, password)
            .then((result)=> {
                self.setAuthToken(result.data.authToken);
                self.setUserId(result.data.userId);
                return null
            })
